...
export function InventoryStatusBadge({ label, tone = 'info' }) {
  return <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${toneMap[tone]}`}>{label}</span>;
}