...
import { Boxes, ChevronRight, Eye, Plus } from 'lucide-react';

export function MaterialHeader({ onGoHome, onGoAdmin, onCreate, readOnly = false }) {
  return (
    <>
      {/* Breadcrumb */}
      ...
      <section className="rounded-[12px] border border-[#D1D5DB] bg-white p-6 shadow-sm">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
          <div>
            ...
            {readOnly ? (
              <p className="mt-2 max-w-3xl text-sm text-gray-600">
                Consulte las referencias de materiales activos del sistema. La creación y edición de materiales está reservada al Administrador del Sistema.
              </p>
            ) : (
              <p className="mt-2 max-w-3xl text-sm text-gray-600">
                Gestione el catálogo de materiales del sistema, mantenga referencias vigentes y deje la estructura lista para futura integración con backend real.
              </p>
            )}
          </div>
          ...
          {readOnly && (
            <div className="rounded-[12px] border border-[#E5E7EB] bg-[#F9FAFB] p-4 xl:min-w-[280px]">
              <div className="flex items-center gap-2 text-[#1F4E79]">
                <Eye size={16} />
                <p className="text-sm font-medium">Modo consulta</p>
              </div>
              <p className="mt-1 text-xs text-gray-500">
                Solo puede visualizar y consultar el detalle de cada material. Para editar, contacte al administrador.
              </p>
            </div>
          )}
        </div>
