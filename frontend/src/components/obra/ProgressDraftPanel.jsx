...
export function ProgressDraftPanel({ selectedRubro, values, onReset }) {
  return (
    <section className="rounded-[12px] border border-[#D1D5DB] bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Revise el contexto antes de registrar el avance.</p>
        </div>
        <button
          type="button"
          onClick={onReset}
      ...
      <div className="mt-4 space-y-3 text-sm text-gray-600">
        <p>
          Rubro actual: <span className="font-semibold text-[#2F3A45]">{selectedRubro ? `${selectedRubro.code} · ${selectedRubro.description}` : 'Sin seleccionar'}</span>
        </p>
        <p>
        ...
        <div className="rounded-[12px] border border-[#D1D5DB] bg-[#F7F9FC] p-3">
          ...
          <p className="mt-2 text-sm text-[#2F3A45]">{values.notes.trim() || 'No hay notas capturadas en este borrador.'}</p>
        </div>
      </div>
    </section>
  );
