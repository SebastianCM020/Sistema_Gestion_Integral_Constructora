...
export function ProjectContextHeader({ currentProject, hasMultipleProjects, pendingSyncCount, onGoHome, onOpenProjectModal }) {
  return (
    <section className="rounded-[12px] border border-[#D1D5DB] bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-medium text-[#1F4E79]">Registro mobile-first para campo</p>
          <h1 className="mt-1 text-2xl font-semibold text-[#2F3A45]">Avance de obra</h1>
          <p className="mt-2 text-sm text-gray-600">Seleccione proyecto y rubro, valide presupuesto y registre el avance con el menor esfuerzo posible desde móvil.</p>
        </div>
        <ProgressPendingSyncBadge count={pendingSyncCount} />
      </div>
      ...
      <div className="mt-5 rounded-[12px] border border-[#D1D5DB] bg-[#F7F9FC] p-4">
        ...
        {!hasMultipleProjects ? (
          <p className="mt-3 text-sm text-gray-500">Solo tiene un proyecto asignado para esta jornada.</p>
        ) : null}
      </div>
    </section>
  );
