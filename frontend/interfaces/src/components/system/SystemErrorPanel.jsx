...
export function SystemErrorPanel({ title, description, onRetry, onGoHome, onDismiss }) {
  return (
    <section className="rounded-[12px] border border-[#DC2626]/15 bg-white p-5 shadow-sm">
      <div className="flex items-start gap-4">
        <div className="flex h-12 w-12 items-center justify-center rounded-[12px] bg-[#DC2626]/10 text-[#DC2626]"><AlertTriangle size={22} /></div>
        <div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        {onDismiss ? <button type="button" onClick={onDismiss} className="inline-flex h-[44px] items-center justify-center rounded-[12px] border border-[#D1D5DB] px-4 text-sm font-medium text-[#2F3A45] hover:bg-[#F7F9FC]">Cerrar</button> : null}
      </div>
    </section>
  );
}