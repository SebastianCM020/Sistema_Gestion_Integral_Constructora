...
import { ProjectAssignmentSelector } from './ProjectAssignmentSelector.jsx';

export function ChangeProjectModal({ projects, currentProjectId, hasDraft, onCancel, onConfirm }) {
  const [selectedProjectId, setSelectedProjectId] = useState(currentProjectId);

  return (
    <ModalShell
      title="Cambiar proyecto"
      description="Seleccione uno de los proyectos asignados para continuar con el registro de avance."
      onClose={onCancel}
      ...
      <div className="space-y-5">
        {hasDraft ? (
          <div className="rounded-[12px] border border-[#F59E0B]/20 bg-[#F59E0B]/10 px-4 py-3 text-sm text-[#9A6700]">
            Cambiar de proyecto limpiará el rubro seleccionado y el borrador actual para evitar inconsistencias de contexto.
          </div>
        ) : null}

        <ProjectAssignmentSelector projects={projects} selectedProjectId={selectedProjectId} onSelect={setSelectedProjectId} />
