...
Write-Host $t1.Substring(0, [Math]::Min(6000, $t1.Length))

Write-Host "`n=== INFORME EJECUCION PRUEBAS ===" -ForegroundColor Cyan
$t2 = Get-DocxText "$base\Informe_Ejecucion_Plan_Pruebas_Verificacion_Validacion_Integracion_ICARO_v3.docx"
Write-Host $t2.Substring(0, [Math]::Min(6000, $t2.Length))
